...
- **Stage**: Stage 04 — Staff (Audit & Architecture Checkpoint Correction)
...
- **CORRECTION_BASE_SHA**: `2ad6b23357bcf49d1224a34e3cf4219c2122359f`
...
- **Current Status**: **STAGE 04 AUDIT CORRECTIONS READY FOR INDEPENDENT REVIEW — PUSHED AND STOPPED**.
...
4. **Zero renderer mutations to dormant or cross-module boundaries**: Attendance, Schedule, Payroll, Finance, and Web Onboarding are strictly excluded.
5. **Read-only initial vertical slice**: Stage 04 implementation begins with an authoritative read-only staff roster and operational inspector. No mutations are authorized for Prompt 2 implementation.
6. **No fake or speculative local fallback data**: All state surfaces reflect genuine database and transport responses.
...
| `is_active`            | `boolean`     | NO       | Raw operational boolean. Governs active roster inclusion vs onboarding / pending states (see detailed status semantics below).                                                 |
...
The database column `is_active` is a raw boolean, but the hosted application interprets operational status via `getStaffStatus(member)` in `src/components/features/staff/staff-management-utils.ts`:
...
4. **`inactive` ("Inactive")**: Legacy / theoretical category in `StaffStatus`. True deactivation in hosted code either sets `is_active = false` on an existing active staff member or archives the row (`archived_at`).

> [!IMPORTANT]
> A staff status badge must NOT label all `is_active = false` records as "Deactivated". Stage 04 must mirror the hosted status derivation to distinguish active employees from unonboarded invitees and awaiting approvals.

### C. Secondary Data Model (`public.staff_services`)
...
| **Fetch Staff Capabilities**     | Database RLS         | `supabase.from('staff_services').select('service_id, services(...)')`   | Supabase Session  | Operational roles                                               | Branch staff only                                  | `staff_id: string`                                                 | Array of assigned service capability records     | Rendered inside the Staff Inspector panel Services tab.                                     |
| **Search / Filter Staff**        | Client / Memory      | Local filtering over loaded branch roster                               | Supabase Session  | Operational roles                                               | Current branch                                     | `search: string`, `role: string`, `status: string`, `type: string` | Filtered array of `StaffMemberRow`               | Real-time search by name, nickname, or phone; filter by role, staff type, or status.        |
...
| **Populated Success**                    | Authoritative staff query returns >= 1 rows for authorized branch     | Displays truthful KPI summary cards, populated DataGrid table, and auto-selects the first staff member in the Inspector card.                                     |
...
| **Selected Staff Member**                | User clicks DataGrid row or auto-selects on load                      | Highlights row in DataGrid, opens `StaffInspectorCard` showing operational profile, contact details, status badge, and assigned service capabilities.             |
...
| **Filter by Role / Type / Status**     | Local State | YES                   | Any operational role | Filters DataGrid rows by `system_role`, `staff_type`, or derived status (`active`/etc.). |
...
| **Inspect Service Capabilities**       | Read-Only   | YES                   | Any operational role | Renders list of qualified services for selected staff member in Inspector Services tab.  |
...
1. **Service Layer (`src/lib/staff-service.ts`)**:
   ...
   - `fetchBranchStaff(branchId: string)`: Queries `public.staff` with joined `branches` and `staff_services(service_id, services(id, name, duration_minutes, price))` ordered by `tier, full_name`.
...
2. **Components (`src/components/staff/`)**:
   ...
   - `StaffKpiSummary.tsx`: 4 truthful operational metrics:
     - **Total Staff**: Total staff records returned for the current branch.
     - **Active Providers**: Count of active (`is_active === true`) staff in service-capable types (`therapist`, `nail_tech`, `aesthetician`, `salon_head`).
     - **Support & Front Desk**: Count of staff in operational support roles (`crm`, `driver`, `utility`, `csr_head`, `csr_staff`, `csr`).
     - **Department Heads**: Count of staff with supervisor / department lead flag (`is_head === true`).
   - `StaffListCard.tsx`: Canonical DataGrid table with columns (Staff Member, Role, Staff Type, Skill Tier, Phone, Status) and active role/type/status filter controls.
   - `StaffInspectorCard.tsx`: Two-tab inspector (Profile, Services) displaying avatar, contact info, derived status badge, and assigned service capabilities.
