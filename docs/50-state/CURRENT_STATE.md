...
Stage 04 (Staff): **STAGE 04 FINAL FUNCTIONAL TRUTH CORRECTION READY FOR INDEPENDENT REVIEW — PUSHED AND STOPPED**. Functional read-only workspace correction complete.
...
- **Final Functional Truth Deliverables**:
  ...
  - Strict nested service capability validation in `extractCapabilities` requiring `services.length === 1` for relation arrays, rejecting `undefined`, and preventing fabricated `"Unnamed Service"` labels.
...
- **Verification Baseline**:
  - 256 passing vitest tests across 13 test files.
